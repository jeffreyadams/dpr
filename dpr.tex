...
\begin{dfn}
	...
	\begin{enumerate}
		\item $\pi$ is called \emph{square-integrable}, or \emph{in the discrete series}, if every matrix coefficient of $\pi$ is square-integrable, i.e. belongs to $L^2(\mc{G})$.
  		...
  		\item An irreducible representation $\pi$ is called  \emph{essentially square-integrable}, or  \emph{in the essentially discrete series} (\emph{eds} for short), if there exists a character $\chi : \mc{G} \to \C^\times$ such that $\pi \otimes \chi$ is relatively square-integrable.
	\end{enumerate}
\end{dfn}
...
\end{lem}
\begin{proof}
  First we prove that if $\pi$ is an irreducible representation then  $\pi|_{\mc{G}^{\natural}}$ is unitary if and only if $\pi\otimes\omega$ is unitary for some one-dimensional representation $\omega$.
  If $\pi\otimes\omega$ is unitary then, 
since  $\mc{G}^{\natural}$ is contained in the commutator subgroup,  $\omega$ is trivial when restricted to $\mc{G}^{\natural}$, so $\pi|_{\mc{G}^\natural}$ is unitary. For the other direction suppose $\pi|_{\mc{G}^{\natural}}$ is unitary. It suffices to show that any character $\chi : Z(\mc{G}) \to \R_{>0}$ extends to a character $\mc{G} \to \R_{>0}$:
for then $\pi\otimes |\chi_\pi|^{-1}$ is unitary where $\chi_\pi$ is the central character of $\pi$.

To show the claim we note that $Z(\mc{G}) \cap \mc{G}^\natural$ is a finite group, because the center of $G_\tx{sc}(\R)$ is finite. Hence $\chi$ is trivial on this restriction, and thus extends to a character of $\mc{G}^\natural \cdot Z(\mc{G})$, trivial on $\mc{G}^\natural$. Since $\R_{>0}$ is uniquely divisible and $\mc{G}^\natural \cdot Z(\mc{G})$ is of finite index in $\mc{G}$, this extension further extends to $\mc{G}$.

Suppose $\pi|_{\mc{G}^\natural}$ is square integrable. In particular this restriction is unitary, so by the previous discussion $\pi\otimes\omega$ is unitary for some $\omega$,
so $\pi$ is in the relative discrete series. Conversely assume $\pi$ is essentially discrete series. After tensoring with a character we may assume $\pi$ is unitary. Since $\mc{G}^\natural Z(\mc{G})$ has finite index,
the restriction to $\mc{G}^\natural Z(\mc{G})$ is   of finite length, unitary and therefore semisimple. These properties also hold upon restriction to $\mc{G}^\natural$
since $ mc{G}$ is central. Furthermore the matrix coefficients of $\pi|_{\mc{G}^\natural}$ are the restrictions of the matrix coefficients of $\pi$, which are in $L^2(\mc{G}^\natural)$.


\end{proof}

\begin{rem}Some authors use say an irreducible representation $\pi$ (not necessarily unitary) is in the {\emph relative discrete series} if its matrix coefficients are in $L^2$ when restricted  to the commutator subgroup. By the Lemma (and its proof) this is  equivalent to being essentially discrete series.
\end{rem}
