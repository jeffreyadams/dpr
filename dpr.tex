...
\begin{dfn}
	Let $\pi$ be an irreducible admissible representation of $\mc{G}$ (or, more generally, a finite direct sum of such).
	\begin{enumerate}
		\item $\pi$ is called \emph{square-integrable}, or \emph{in the discrete series}, if every matrix coefficient of $\pi$ is square-integrable, i.e. belongs to $L^2(\mc{G})$.
  		\item $\pi$ is called \emph{relatively square-integrable}, or in the \emph{in the relative discrete series}, if the central character of $\pi$ is unitary and every matrix coefficient of $\pi$ is square-integrable modulo the center of $\mc{G}$, i.e. lies in $L^2(\mc{G}/Z(\mc{G}))$.
  		\item $\pi$ is called essentially \emph{square-integrable}, or  \emph{in the essentially discrete series} (\emph{eds} for short), if there exists a character $\chi : \mc{G} \to \C^\times$ such that $\pi \otimes \chi$ is relatively square-integrable.
	\end{enumerate}
\end{dfn}
...
\begin{lem}
	Let $\pi$ be an irreducible (not necessarily unitary) representation of $\mc{G}$ on a Hilbert space \warn{do we need that restriction?}. Then $\pi$ is essentially square-integrable if and only its restriction to $\mc{G}^\natural$ (which is semi-simple and of finite length) is square-integrable.
\end{lem}
\begin{proof}
	The restriction of $\pi$ to $\mc{G}^\natural \cdot Z(G)$ is semi-simple and of finite length \warn{Jeff: can you find a reference for that?}. Moreover, since $\pi$ has a central character, \warn{Jeff: Can you provide a reference for that?}, the restriction of $\pi$ to $\mc{G}^\natural$ is semi-simple and of finite length. If $\pi$ is assumed to be essentially square-integrable, then the restriction to $\mc{G}^\natural$ of its matrix coefficients lie in $L^2(\mc{G}^\natural)$. But these restrictions are the matrix coefficients of $\pi|_{\mc{G}^\natural}$.
	
	In the converse direction, we first treat the case that $\pi$ has a unitary central character. If $\pi|_{\mc{G}^\natural}$ is square-integrable, then the matrix coefficients of $\pi$ lie in $L^2(\mc{G}^\natural)$. They transform by a unitary character under $Z(\mc{G})$, so they lie in $L^2(\mc{G}^\natural \cdot Z(\mc{G})/Z(\mc{G}))$. But since $\mc{G}^\natural \cdot Z(\mc{G})$ is of finite index in $\mc{G}$, this implies that the matrix coefficients lie in $L^2(\mc{G}/Z(\mc{G}))$, hence that $\pi$ is relatively square-integrable.

	Finally, it suffices to show that any character $\chi : Z(\mc{G}) \to \R_{>0}$ extends to a character $\mc{G} \to \R_{>0}$. Indeed, this would imply that any representation of $\mc{G}$ which has a central character $\omega : Z(\mc{G}) \to \C^\times$ can be twisted by a character of $\mc{G}$ to ensure that $\omega$ becomes unitary, namely by the extension of $|\omega| : Z(\mc{G}) \to \R_{>0}$. To show the claim we note that $Z(\mc{G}) \cap \mc{G}^\natural$ is a finite group, because the center of $G_\tx{sc}(\R)$ is finite. Hence $\chi$ is trivial on this restriction, and thus extends to a character of $\mc{G}^\natural \cdot Z(\mc{G})$, trivial on $\mc{G}^\natural$. Since $\R_{>0}$ is uniquely divisible and $\mc{G}^\natural \cdot Z(\mc{G})$ is of finite index in $\mc{G}$, this extension further extends to $\mc{G}$.
\end{proof}
...
The resulting map $\tau : \mf{g} \to \mc{D}(G)$ of complex vector spaces translates the Lie bracket on $\mf{g}$ to the commutator bracket of the associative algebra $\mc{D}(G)$, and thus extends to a homomorphism of $\C$-algebras
\[ \tau : \mc{U}(\mf{g}) \to \mc{D}(G). \]
This homomorphism is injective and its image is precisely the subalgebra of left-invariant operators. The restriction of $\tau$ to the center $\mc{Z}(\mf{g})$ of $\mc{U}(\mf{g})$ induces an isomorphism between $\mc{Z}(\mf{g})$ and the subalgebra of differential operators that are both left- and right-invariant.

The Harish-Chandra isomorphism is an isomorphism $\gamma : \mc{Z}(\mf{g}) \to \mc{S}(\mf{t})^\Omega$ of commutative $\C$-algebras that identifies the center $\mc{Z}(\mf{g})$ of the universal enveloping algebra $\mc{U}(\mf{g})$  of $\mf{g}$ with the invariants under the Weyl group $\Omega=\Omega(T,G)$ in the symmetric algebra $\mc{S}(\mf{t})$, the latter being equal to the universal enveloping algebra $\mc{U}(\mf{t})$ of $\mf{t}$.

The result of Harish-Chandra that we want to recall here is the following.
...
\begin{pro} \label{pro:diff}
	...
	\[ |D_T|^{1/2} \cdot (\tau(z)f)|_{T'} =  \tau(\gamma(z))(f|_{T'} \cdot |D_T|^{1/2}). \]
\end{pro}

This result is not stated in exactly this form in the works of Harish-Chandra, but can be extracted as follows.

Let $\mc{I}(G) \subset \mc{D}(G)$ denote the subalgebra of left-invariant differential operators, so that $\tau : \mc{U}(\mf{g}) \to \mc{I}(G)$ is an isomorphism. The first step is to recall that Harish-Chandra constructs in \cite[\S4]{HC_characters} a family of retractions $\mc{D}(G) \to \mc{I}(G)$ to the inclusion $\mc{I}(G) \to \mc{D}(G)$. This family is indexed by the set of points of $G$, and the retraction $\mc{D}(G) \to \mc{I}(G)$ associated to $y \in G$ is denoted by $D \mapsto D_y$. The left-invariant operator $D_y$ is called the ``local expression of $D$ at $y$'', and has the defining property that 
\begin{equation} \label{eq:diff1}
	\forall f \in \mc{C}^\infty(G) : (Df)(y) = (D_yf)(y).
\end{equation}
This is \cite[\S4, Corollary to Lemma 13, page 112]{HC_characters}.

The next step contains the passage from $G$ to $T$. Consider a strongly regular element $a \in T$. Let $U_G$ be an open neighborhood of $a$ in $G$ and let $U_T = T' \cap a^{-1}U_G$, an open neighborhood of $1$ in $T$. In \cite[\S 5]{HC_inv_eigen} Harish-Chandra constructs a map
\[ \delta_a : \mc{D}(U_G) \to \mc{D}(U_T)\]
which is characterized by the property 
\[ \forall y \in U_T : \delta_a(D)_y = \alpha_y(D_{ay}), \]
where $\alpha_y : \mc{U}(\mf{g}) \to \mc{U}(\mf{t})$ is constructed in \cite[\S5, Corollary 1, page 463]{HC_inv_eigen} for any $y \in T$ so that $ay$ is still regular.

The final step is to connected the operator $\delta_a(\tau(z))$, for $z \in \mc{Z}(\mf{g})$, to the operators $\tau(z)$ and $\tau(\gamma(z))$. More precisely, \cite[\S6, Lemma 13, page 466]{HC_inv_eigen} states that
\begin{equation} \label{eq:diff2}
\delta_a(\tau(z)) = |\nu_a|^{-1/2} \circ \tau(\gamma(z)) \circ |\nu_a|^{1/2},	
\end{equation}
where $|\nu_a|$ is the operator of multiplication by the smooth function $|\nu_a(h)| = |\det(\tx{Ad}(ah)^{-1}-1|\mf{g}/\mf{t})$ defined on the set $T \cap a^{-1}G'$. We note that the notation $\tau$ has been suppressed in loc. cit. On the other hand, \cite[\S8, Lemma 18, page 466]{HC_inv_eigen} provides the identity
\begin{equation} \label{eq:diff3}
	D(f)(ay) = \delta_a(D)_y(f)(ay)
\end{equation}
for any $y \in T \cap a^{-1}G'$ and any $f \in \mc{C}^\infty(U_G)$ that is locally invariant. Here local invariance is defined to mean that the map $\R \to \R, t \mapsto f(\exp(tX)y\exp(-tX))$ has trivial derivative at $t=0$  for all $X \in \mf{g}$ and $y \in U_G$, cf. \cite[\S8]{HC_inv_dist}. In particular, the restriction to $U_G$ of any conjugation-invariant function on $G'$ is locally invariant.

We mention here that \eqref{eq:diff3} is not exactly the statement of \cite[Lemma 18]{HC_inv_eigen}. Rather, in that statement $\delta_a(D)$ appears in place of $\delta_a(D)_y$. This is however a typographical error. Indeed, $\delta_a(D)$ is a differential operator on $U_T$, but $ay$ does not belong to $ay$, so the term $\delta_a(D)(f)(ay)$, which in the notation of loc. cit. is written as $f(ay;\delta_a(D))$, is not defined. On the other hand, the last line of the proof of that lemma concludes
\[ D_{ay}f(ay) = \delta_a(D)_yf(ay), \]
which is exactly \eqref{eq:diff3}, when we note that by definition of the local expression we have $D_{ay}f(ay)=Df(ay)$.
...
\begin{proof}[Proof of Proposition \ref{pro:diff}]
	Let $a \in T'$. We apply \eqref{eq:diff3} with $1 = y \in T \cap a^{-1}G'$ and $D=\tau(z)$ and obtain
	\[ (\tau(z)f)(a) = \delta_a(\tau(z))_1(f)(a) = [L_{a^{-1}}\circ \delta_a(\tau(z))_1](f)(1).\]
	The operator $\delta_a(\tau(z))$ is left-invariant, hence commutes with the operator $L_{a^{-1}}$, and using the defining property of the local expression 
	we see
	\[ [L_{a^{-1}}\circ \delta_a(\tau(z))_1](f)(1) = \delta_a(\tau(z))_1(L_{a^{-1}}f)(1) = \delta_a(\tau(z))(L_{a^{-1}}f)(1). \]
	We now apply \eqref{eq:diff2} and see
	\[ \delta_a(\tau(z))(L_{a^{-1}}f)(1) = [|\nu_a|^{-1/2} \circ \tau(\gamma(z)) \circ |\nu_a|^{1/2}](L_{a^{-1}}f)(1). \]
	A direct computation shows that $|\nu_a|^{1/2} \circ L_{a^{-1}}=L_{a^{-1}} \circ |\nu|^{1/2}$, where $|\nu|$ is the operator of multiplication by the smooth function $|\nu(h)|=|\det(\tx{Ad}(h)^{-1}-1|\mf{g}/\mf{t})|$, which is nothing else but the function $|D_T|$. Using the left-invariance of $\tau(\gamma(z))$ we see that 
	\[ [|\nu_a|^{-1/2} \circ \tau(\gamma(z)) \circ |\nu_a|^{1/2}](L_{a^{-1}}f)(1) = [|\nu|^{-1/2} \circ \tau(\gamma(z)) \circ |\nu|^{1/2}](f)(a).\]
	Putting these equations together one obtains the desired proof.
\end{proof}
