...
\warn{this proof is not finished}
  ...
  \begin{proof}
	...
	Let $T \subset G$ be the centralizer of $\delta$ and let $S \subset H$ be the centralizer of the image $\gamma \in H(\R)$ of $\dot\gamma$. Then $\gamma \in Z_H(\R) \cdot S(\R)'$, where $S(\R)'$ is the image of $S_\tx{sc}(\R) \to S(\R)$. There is a unique admissible isomorphism $j : S \to T$ mapping $\gamma$ to $\delta$.
  ...
  \end{proof}
	...
	\begin{proof}
		...
		To see boundedness, we note first that we have already normalized the characters by $|D_T|^{1/2}$, so we need to show that the right-hand side is bounded. For any fixed $\delta$, there are only finitely many $\dot\gamma$ with $\Delta(\dot\gamma,\delta) \neq 0$, and in fact their number is bounded by $|\Omega(T,G)|$. The term $\Delta_I(\dot\gamma,\dot\delta)$ is obtained by evaluating a character of a Galois cohomology group on an element of that group. The result is a root of unity. The term $\Delta_{III}(\dot\gamma,\dot\delta)$ is a \warn{unitary!!!} character of a cover of the torus $T(\R)$. 
