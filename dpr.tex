...
Since we don't need this we refer to \cite[Lemma 14.14]{ABV92}.
...
Now fix a non-trivial unitary character $\Lambda$ of $\R$. Then $\eta(\P,\Lambda)=\Lambda\circ \eta_{\P}$ is a non-degenerate unitary character of $N(\R)$.
The $G(\R)$-conjugacy class of $\eta(\P,\Lambda)$ only depends on the $G(\R)$-conjugacy class of  $\P$.  Write $\w(\P,\Lambda)$ for the Whittaker model
$[(B,\eta(\P,\Lambda)]$.

\begin{lem} \label{lem:w1}
Fix $\Lambda$. The map $\P\mapsto \w(\P,\Lambda)$ is a bijection from $G(\R)$-conjugacy classes of real pinnings to Whittaker models.

Set $\Lambda(x)=e^{\pi ix}$. Then $\w(\P,\Lambda)=\w_{\overline X}$, where $\P=(T,B,\{X_\alpha\})$ and $\overline X=i\sum_{\alpha\in\Delta}X_{-\alpha}$,
...
therefore
\[ \eta_{\bar X}(\exp(Y)) = e^{\kappa(Y,\bar X)}=e^{2\pi i \kappa(Y,X_-)} = \Lambda(\sum_{\alpha \in\Delta}y_\alpha) = \eta(\P,\Lambda)(\exp(Y)). \]
\end{proof}
...
\warn{Uniqueness isn't so easy. There is a reference: \cite[Lemma 14.14]{ABV92}. If you want to include a proof, 
  I think the best one uses the associated variety argument, which was deleted as not needed but could be put back.
  Then: the associated variety of a discrete series is well known (reference...) to be a single orbit, so
  by the bijection the same holds for the Whittaker model}
\end{proof}
...
{\bf Alternative proof of 1}
The image of $\C^\times$ is connected, abelian, and consists of semisimple elements. It follows [reference?] that $\hat L=\Cent_{\hat G}(\phi(\C^\times))$ is a Levi subgroup.
In particular $\hat L$ is connected, reductive, and contains a maximal torus $\hat S$ of $\hat G$. Since $\phi(\C^\times)$ is contained in the identity component of the center of $\hat L$, it is contained in $\hat S$.
Choose an element $g\in \phi(W_\R)\backslash \phi(\C^\times)$. Then $g$ normalizes $\hat L$, and since $g^2\in \phi(\C^\times)$ conjugation by $g$ acts as an involution on $\hat L$.
Therefore $\hat L$ contains a maximal torus $\hat T$ which is fixed by conjugation by $g$
(this is well known, or apply \cite[Theorem 5.16]{SS70}).
Then $\hat T$ is a maximal torus of $\hat G$; containing $\phi(\C^\times)$ (since the latter is contained in the center of of $\hat L$);
fixed by the action of $g$; and therefore normalized by $\phi(\C^\times)$.
...
Let $S(F)_G$ be the double cover of $S(F)$ associated to the subset $R(S,G) \subset X^*(S)$. It is the same double cover as in \S\ref{sub:fac}.
\warn{I don't think Section 4.1 is the correct reference. Section 2.8?}.Now we have another double cover, called $S(F)_x$, coming from the pull-back of $x : \tilde\pi_1(G) \to \mu_n(\C)$ under the natural map $\tilde\pi_1(S) \to \tilde\pi_1(G)$. Each admissible embedding $j : S \to G$ lifts naturally to an embedding $S(\R)_x \to G(\R)_x$.
