...
\warn{this proof is not finished}
  ...
  \underline{Step 1:} Both sides are supported on $Z_G(\R) \cdot G_\tx{der}(\R)^0$, and transform by the same character of $Z_G(\R)$. Since $G_\tx{sc}(\R) \to G_\tx{der}(\R)^0$ is surjective, it is enough to compare them after pulling back to $G_\tx{sc}(\R)$. This allows us to assume that $G$ is semi-simple and simply connected.

  \begin{proof}
	  We first claim that the character of a discrete series representation of $G(\R)$ is supported on $Z_G(\R) \cdot G_\tx{der}(\R)^0$. \warn{Jeff, can you prove that?}

	  It follows that the function $\Theta_\varphi^{\mf{w},s}$ is supported on $Z_G(\R) \cdot G_\tx{der}(\R)^0$ and transforms under $Z_G(\R)$ by the common central character $\omega_\varphi : Z_G(\R) \to \C^\times$ of all members of $\Pi_\varphi(G)$.
	  
	Applying the same reasoning to $H$ we see that the function $S\Theta_{\varphi_\pm}$ is supported on $Z_{H(\R)_\pm} \cdot H(\R)_\pm'$, where $H(\R)_\pm'$ is the image of the natural splitting $H_\tx{sc}(\R) \to H(\R)_\pm$.
  ...
  \end{proof}
	...
	\underline{Step 2}: The left side is a tempered invariant eigendistribution. Show that the right side is also a tempered invariant eigendistribution.
	...
	\begin{proof}
		...
		The distribution is \emph{invariant} if it is invariant under the action of $G(\R)$ by conjugation on itself, hence on functions. 
		
		It is an \emph{eigendistribution}, if it transforms under a character $\chi : \mf{z} \to \C$ for the action of the center $\mf{z}$ of the universal enveloping algebra of the Lie algebra of $G$. 

		By Harish-Chandra's regularity theorem \cite[Theorem 8.4.1]{WallachRed1}, an invariant eigendistribution $T$ is represented by a an analytic function $F_T$ on $G(\R)_\tx{sr}$ which is locally integrable on $G(\R)$. This distribution is tempered if and only if the function $F_T$ satisfies the following inequality, see \cite[Theorem 8.6.1]{WallachRed1}
		\[ |D_T(x)|^{1/2} \cdot |F_T(x)| \leq C(1+\log||x||)^d \]
		for some fixed constants $c \geq 0$, $D \geq 0$, and all strongly regular $x \in G(\R)$.

		It is known that the character of a discrete series representation of a semi-simple group is a tempered invariant distribution, see \cite[Lemma 8.5.2]{WallachRed1}. In fact, it is an eigendistribution for the infinitesimal character of $\pi$, which is the $\Omega(S,G)$-orbit $[d\tau]$ of $d\tau_\pi$ \warn{Reference?}. Therefore, the virtual sum $\Theta_\varphi^{\mf{w},s}$ is also a tempered invariant eigendistribution with infinitesimal character $[d\tau]$.

		We now claim that the right-hand side of the identity of Theorem \ref{thm:main2} is also a tempered invariant eigendistribution with the same infinitesimal character. Invariance (under conjugation by $G(\R)$) follows from the corresponding property of the transfer factor $\Delta(\dot\gamma,\delta)$ in the variable $\delta$.

		To see temperedness, we note first that we have already normalized the characters by $|D_T|^{1/2}$, so the estimate we need is $|F_T(\delta)| \leq C(1+ \log||\delta||_G)^d$, where $F_T$ is the function on the right hand side of the identity of Theorem \ref{thm:main2}. We know $|S\Theta(\dot\gamma)| \leq C(1+\log||\gamma||_H)^d$. For any fixed $\delta$, there are only finitely many $\dot\gamma$ with $\Delta(\dot\gamma,\delta) \neq 0$, and in fact their number is bounded by $|\Omega(T,G)|$. The term $\Delta_I(\dot\gamma,\dot\delta)$ is obtained by evaluating a character of a Galois cohomology group on an element of that group. The result is a root of unity. The term $\Delta_{III}(\dot\gamma,\dot\delta)$ is a \warn{unitary!!!} character of a cover of the torus $T(\R)$. \warn{It remains to relate $||-||_G$ to $||-||_H$.}
	...
	\underline{Step 3:} Apply the Harish-Chandra uniqueness theorem, \cite[Theorem 2]{HCDSI}.
\end{proof}
