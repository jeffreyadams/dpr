...
\warn{this proof is not finished}
  ...
  \underline{Step 1:} Let $G(\R)'$ be the image of $G_\tx{sc}(\R) \to G(\R)$. Both sides are supported on $Z_G(\R) \cdot G(\R)'$, and transform by the same character of $Z_G(\R)$. It is thus enough to compare them after pulling back to $G_\tx{sc}(\R)$. This allows us to assume that $G$ is semi-simple and simply connected.
