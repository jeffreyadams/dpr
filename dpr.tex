...
We now define the Kostant section in $\g^*$. Choose an isomorphism
$\phi : \g \to \g^*$ of $\C$-vector spaces that is 
equivariant for the
action of $G$ and of the Galois group.
Then for $X\in \g^*$ a regular nilpotent element define
...
The proof carries over to $\g^*$ via an isomorphism $\g\simeq \g^*$ as discussed in Section \ref{s:kostant}.
\end{proof}
