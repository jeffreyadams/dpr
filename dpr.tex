...
One class of examples of this set-up arise as $\mc{G}=G(\R)$, where $G$ is a connected reductive $\R$-group. Another class of examples comes from the covers $G(\R)_x$ of such groups constructed in \cite{KalHDC} from characters $x : \bar\pi_1(G) \to \C^\times$.

% \begin{lem} A group $\mc{G}$ satisfying the above conditions is in the Harish-Chandra class.
% \end{lem}
% \begin{proof}
...
% 	Condition (3) may not be satisfied!!
% \end{proof}
...
\begin{pro}
  \label{p:whittaker}
Let $\pi$ be a eds representation,
...
\begin{cor}
	Let $\pi$ be a eds representation. There exists precisely one Whittaker model $\mf{w}$ for which $\pi$ is $\mf{w}$-generic.
\end{cor}
\begin{proof}
