...
\cite[Theorem 19.2]{Bor91} this is independent of the choice of $T$. We define $\w_X$ to be the $G(\R)$-conjugacy class of $(B(\R),\eta_X)$. 
...
$[(B,\eta_{\P,\Lambda})]$. Note that, if we consider an element $i=\sqrt{-1} \in \C$ fixed, we obtain the ``standard'' character $\Lambda : \R \to \mb{S}^1$ given by $x \mapsto e^{ix}$.
...
To do so, fix a non-degenerate, $G(\R)$-invariant bilinear form $\kappa$ on $\g_\tx{der}(\R)$, and extend it by complex-linearity to 
a $G(\C)$-invariant form on $\g_\tx{der}$, still denoted $\kappa$ (for example, we could take the Killing form). Suppose $T\subset B$ are a Cartan and Borel subgroup, both defined over $\R$. 
...
Let $X\in\g$ be a regular nilpotent element. It can be extended to an $SL(2)$-triple $[X,H,Y]$, \cite[Chapter 3]{CM}.  The {\it Kostant Section} $\K(X)$ associated to 
$[X,H,Y]$ is the affine space $X+\Cent_\g(Y)$.
...
Suppose $X\in \g(\R)$ and $\O$ is a regular orbit in $\g(\C)$ which is defined over $\R$, i.e. stable under complex conjugation. Then the unique point in $\K(X)\cap \O$ is also stable under complex conjugation, i.e. contained in $\g(\R)$. The $G(\R)$-orbit of $\K(X)\cap \O$ depends only on the $G(\R)$-orbit of $X$.

We shall now make an analogous construction for $\g^*$. It will use a choice of an isomorphism $\phi : \g \to \g^*$ of $\C$-vector spaces that is equivariant for the action of $G$ and of complex conjugation, but the result will be independent of it. Such an isomorphism exists, and specifying it is equivalent to specifying a non-degenerate bilinear form on $\g$ that is invariant under the actions of $G$ and complex conjugation. We can obtain the latter by taking the Killing form on the derived part of $\g$ and an arbitrary non-degenerate bilinear form on the center of $\g$.

Assuming $\phi$ fixed, given a regular nilpotent element $X\in \g^*$ we define $\K(X)\subset \g^*$ as $\K(X) = \phi(\K(\phi^{-1}(X)))$. The rationality and uniqueness properties of $\K(X)$ are inherited from the analogous properties in the case of $\g$. 

To see that the choice of $\phi$ is irrelevant we decompose  $\g = \mf{z} \oplus \g_1 \oplus \dots \oplus \g_n$, where $\mf{z}$ is the center of $\g$ and $\g_i$ are the simple factors. This decomposition is canonical, up to permuting the $\g_i$.  The dual decomposes accordingly as $\g^*=\mf{z}^*\oplus \g_1^* \oplus \dots \oplus \g_n^*$, where $\mf{z}^*$ is the annihilator of $\mf{g}_1 \oplus \dots \oplus \mf{g}_n$, and also equal to the fixed point space for the coadjoint action of $G$, and $\mf{g}_i^*$ is the annihilator of $\mf{z} \oplus \mf{g}_1 \oplus \dots \oplus \mf{\hat  g}_i \oplus \dots \oplus \mf{g}_n$ (with ``hat'' signifying that we are omitting this summand), and also equal to the dual space of $\mf{g}_i$. The isomorphism $\phi$ breaks up as $\phi_\mf{z}\oplus\phi_1 \oplus \dots \oplus \phi_n$, where $\phi_\mf{z} : \mf{z} \to \mf{z}^*$ is an isomorphism of $\C$-vector spaces equivariant under complex conjugation, and $\phi_i : \g_i \to \g_i^*$ is an isomorphism of complex vector spaces equivariant under $G$ (equivalently, its simple piece $G_i$) and complex conjugation. Of these, $\phi_\mf{z}$ is arbitrary, while $\phi_i$ is uniquely determined up to a complex scalar. Now $X=X_1+\dots+X_n$ and $Y=Y_1+\dots+Y_n$ break up accordingly (neither has a central piece), and we have 
\[ \K(\phi^{-1}(X)) = \mf{z} \oplus \bigoplus (\phi_i^{-1}(X_i)+\Cent_{\g_i}(\phi_i^{-1}(Y_i))),  \]
hence
\[ \K(X) = \mf{z}^* \oplus \bigoplus (X_i+\phi_i(\Cent_{\g_i}(\phi_i^{-1}(Y_i)))). \]
But it is clear that the term $\phi_i(\Cent_{\g_i}(\phi_i^{-1}(Y_i))))$ does not change if we multiply $\phi_i$ by a scalar.
