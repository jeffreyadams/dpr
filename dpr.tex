...
\item the set of generic discrete series representations in an L-packet. \warn{it's too early to talk about $L$-packets here, we will only construct them in the next section. it's also not necessary in my view}
\end{enumerate}
\end{lem}
...
\end{lem}
\begin{proof}
	This follows from the fact that any two elliptic maximal tori of $G$ are conjugate under $G(\R)$ \warn{ref}.
\end{proof}
...
\begin{cor}
	Let $\pi$ be a generic discrete series representation. There exists precisely one Whittaker model $\mf{w}$ for which $\pi$ is $\mf{w}$-generic.
\end{cor}
\begin{proof}
