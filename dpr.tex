...
\item the set of generic discrete series representations in an L-packet.
\end{enumerate}
\end{lem}
...
The second assertion is immediate from Lemma \ref{l:wX}.
...
\begin{pro}
  \label{p:whittaker}
Let $\pi$ be a  generic discrete series representation,
...
Since $\pi$ is generic, $\pi$ is $\w_X$-generic for some $X$. By the previous argument we know $\O_\pi$
meets $\K(X)$ for some $X$; it is enough to show $Y$ is $G(\R)$ conjugate to $X$.
We proceed by contradiction. If $Y$ and $X$ are in different $G(\R)$-orbits, then by Lemma \ref{l:Q} 
$G(\R)\cdot Y=q(G(\R)\cdot X)$ for some $q\ne 1$ in $Q(G(\R))$. Since $Q(G(\R))$ is acting by automorphisms on $G(\R)$, 
we see $\pi^q$ is $\w_Y$-generic. By the previous argument this implies $\O_{\pi^q}$ meets $\kappa(Y)$. 
But we have already assumed $\O_\pi$ meets $\kappa(Y)$. Recall $\kappa(Y)$ meets every regular complex orbit in exactly one point.
Since $\O_\pi$ and $\O_{\pi^q}=q\O_\pi$ are conjugate by $G(\C)$ this implies $\O_{\pi^q}=\O_\pi$. 
