...
The $G(\R)$-conjugacy class of $\eta_{\P,\Lambda}$ only depends on the $G(\R)$-conjugacy class of  $\P$.  Write $\w_{\P,\Lambda}$ for the Whittaker model
$[(B,\eta_{\P,\Lambda}]$.

\begin{lem} \label{lem:w1}
\begin{enumerate}
	\item Fix a non-trivial unitary character $\Lambda$ of $\R$. The map $\P\mapsto \w_{\P,\Lambda}$ is a bijection from $G(\R)$-conjugacy classes of real pinnings to Whittaker models.
...
\begin{lem} \label{lem:g2}
    ...
    Then $\w=\w'$ if and only if $d\tau$ and $d\tau'$ are in the same Weyl chamber. In particular, $\pi$ is generic of a unique Whittaker datum.
\end{lem}
