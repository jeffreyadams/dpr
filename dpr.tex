...
\warn{this proof is not finished}
  ...
  \begin{proof}
		Lemma \ref{lem:charsupp} shows that the character of a discrete series representation of $G(\R)$ is supported on $Z_G(\R) \cdot G(\R)'$. It follows that the function $\Theta_\varphi^{\mf{w},s}$ is supported on $Z_G(\R) \cdot G(\R)'$ and transforms under $Z_G(\R)$ by the common central character $\omega_\varphi : Z_G(\R) \to \C^\times$ of all members of $\Pi_\varphi(G)$. Applying the same reasoning to $H$ we see that the function $S\Theta_{\varphi_\pm}$ is supported on $Z_{H(\R)_\pm} \cdot H(\R)_\pm'$, where $H(\R)_\pm'$ is the image of the natural splitting $H_\tx{sc}(\R) \to H(\R)_\pm$.
	
	Letting $Z_G(\R)$ act on the elliptic maximal torus of $G$ by multiplication, the assumption that the identity of Theorem \ref{thm:main2} holds for elliptic elements implies that both sides of that identity transform under the same character of $Z_G(\R)$.

	To show that the right hand side of that identity is supported on $Z_G(\R) \cdot G_\tx{der}(\R)^0$, consider a strongly regular semi-simple element $\delta \in G(\R)$ for which the right-hand side is non-zero. Thus there exists $\dot\gamma \in H(\R)_\pm$ such that $S\Theta_{\varphi'}(\dot\gamma) \neq 0$ and such that $\Delta(\dot\gamma,\delta)$. 
	
	Let $T \subset G$ be the centralizer of $\delta$ and let $S \subset H$ be the centralizer of the image $\gamma \in H(\R)$ of $\dot\gamma$. Then $\gamma \in Z_H(\R) \cdot S(\R)'$, where $S(\R)'$ is the image of $S_\tx{sc}(\R) \to S(\R)$. There is a unique admissible isomorphism $j : S \to T$ mapping $\gamma$ to $\delta$. The induced isomorphism $X_*(S) \to X_*(T)$ identifies the coroot lattice $Q^\vee(S,H)$ with a sublattice of the coroot lattice $Q^\vee(T,G)$, and hence restricts to a homomorphism $X_*(S_\tx{sc}) \to X_*(T_\tx{sc})$. This means that $j : S(\R) \to T(\R)$ maps $S(\R)'$ into $T(\R)'$. This reduces to showing that $j$ maps $Z_H(\R)$ to $Z_G(\R) \cdot T(\R)'$. 
	
	For this we recall that, since the discrete parameter $\varphi$ factors through $H$, $H$ is an elliptic endoscopic group, i.e. $Z_H/Z_G$ is an anisotropic diagonalizable group. Thus the image of $Z_H(\R)$ in $T(\R)$ lies in $Z_G(\R) \cdot T_c(\R)$, where $T_c(\R)$ is the maximal anisotropic subtorus in $T$.
