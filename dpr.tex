...
It is worth pointing out that Harish-Chandra worked with a slightly different notion of a reductive group, adapted to the language of real Lie groups. His arguments often assumed the group to be connected (in the analytic topology) and \emph{acceptable}, which means that half-sum of the positive roots is analytically integral, i.e. it becomes a character of the complex points of a maximal torus. These conditions are inconvenient when one works with groups of the form $G(\R)$ for a connected reductive algebraic $\R$-group $G$, which are in general neither connected in the analytic topology nor acceptable. However, one can reduce to the setting of Harish-Chandra by the following lemma, based on the fact that $G(\R)$ is both connected and acceptable when $G$ is a simply connected semi-simple algebraic $\R$-group.

\begin{lem} \label{lem:charsupp}
	Let $\pi$ be an irreducible discrete series representation of $G(\R)$. Let $G(\R)'$ denote the image of the natural map $G_\tx{sc}(\R) \to G(\R)$. The restriction of $\pi$ to $Z_G(\R) \cdot G(\R)'$ is a finite direct sum of irreducible representations. If $\pi'$ is any one of them, then its restriction to $G_\tx{sc}(\R)$ is discrete series, and $\pi$ equals the induction of $\pi'$. In particular, the character of $\pi$ is supported on $Z_G(\R) \cdot G(\R)'$.
\end{lem}
\begin{proof}
	Since $\pi$ is irreducible and $Z_G(\R)\cdot G(\R)'$ is of finite index in $G(\R)$, the restriction of $\pi$ is a finite direct sum of irreducible representations. They all share the same central character, namely that of $\pi$. Therefore, each of them remains irreducible upon restriction to $G(\R)'$. If $\pi'$ is one of them, the square-integrability of its matrix coefficients is inherited from that of $\pi$, and its inflation to $G_\tx{sc}(\R)$ is still square-integrable. By Harish-Chandra's theory, there exists an anisotropic maximal torus $S_\tx{sc}$ of $G_\tx{sc}$ and a regular character $\lambda$ of $S_\tx{sc}(\R)$, with the property that if $\rho$ is half of the sum of the positive roots for the chamber containing $\lambda$, then $\lambda-\rho$ still lies in that chamber.

	The group $G(\R)$ acts by conjugation on $G(\R)'$, hence also on its representations. We claim that the stabilizer of $\pi'$ for this action is $Z_G(\R) \cdot G(\R)'$. Indeed, if an element $g \in G(\R)$ stabilizes $\pi$, it stabilizes the $G(\R)'$-orbit of $\lambda$, hence we can modify it to assume it fixes $\lambda$. Then it lies in the centralizer of $\lambda$, which is an elliptic maximal torus $S(\R)$. Both $S_\tx{ad}(\R)$ and $S_\tx{sc}(\R)$ are compact tori, hence connected, hence the isogeny $S_\tx{sc}(\R) \to S_\tx{ad}(\R)$ is surjective. It follows that the element $g$ lies in $Z_G(\R) \cdot S(\R)'$, where $S(\R)'$ is the image of $S_\tx{sc}(\R) \to S(\R)$.

	We have proved that the stabilizer of $\pi'$ in $G(\R)$ is $Z_G(\R) \cdot G(\R)'$. Therefore the induction of $\pi'$ to $G(\R)$ is irreducible, and since there is a non-trivial map between it and $\pi$, this map must be an isomorphism.
\end{proof}

With this translation to the setting of connected acceptable groups, the discussion of discrete series representations and their characters comes from \cite[Theorem 3]{HCDSI}.
...
Suppose $X\in i\g(\R)$ is a regular nilpotent element. Let $\overline
B$ be the unique Borel subgroup containing $X$. Then $\overline B$ is
defined over $\R$. Let $B$ be an $\R$-Borel subgroup that is opposite
to $\bar B$, i.e. such that $B \cap \bar B$ is a maximal torus.
The Whittaker datum defined by $X$ is  $\w_X=[(B,\eta_X)]$.

We need to show that $\w_X$ is well defined, i.e. the conjugacy class of $(B,\eta_X)$ is independent of the choice of $B$.
...
Any Whittaker datum $[(B,\eta)]$ is equal to $\w_X$ for some regular nilpotent element $X\in i\overline\n(\R)$.
Also the map $X\rightarrow \w_X$ is $G_\mathrm{ad}(\R)$ equivariant. 
...
$[(B,\eta_{\P,\Lambda})]$. The map $\P\mapsto \w_{\P,\Lambda}$ is $G_{\mathrm{ad}}(\R)$ equivariant.

We want to show the maps $X\mapsto \w_X$ and $\P\mapsto \w_{\P,\Lambda}$ are bijections to the set of Whittaker models. We already know they are $G_{\mathrm{ad}}(\R)$ equivariant.
It is helpful to introduce a finite group which parametrizes these sets.
Let
$$
Q(G(\R))=G_{\mathrm{ad}}(\R)/\mathrm{ad}(G(\R)).
$$
This is a finite abelian $2$-group, which
acts by automorphisms on $G(\R)$.


\begin{lem}
\label{l:Q}
	$Q(G(\R))$ acts simply transitively on:
\begin{enumerate}
  \item[(1)] the set of $G(\R)$-conjugacy classes of real pinnings;
  \item[(2)] the set of regular nilpotent orbits in $\g(\R)$;
  \item[(3)] the set of Whittaker data;
\end{enumerate}
The maps $\P\mapsto \w_{\P,\Lambda}$ and  $X\mapsto \w_X$  are $Q(G(\R))$ equivariant bijections from (1) to (3) and (2) to (3), respectively.
\end{lem}

See \cite[Lemma 14.14]{AV92}.

\begin{proof}
  For (1) using the fact that all real Borel subgroups are $G(\R)$-conjugate, this reduces to the fact, given $(B,T)$,  $T_{\mathrm{ad}}(\R)$ acts simply transitively
on the sets of non-zero simple root vectors. 
If the simple root $\alpha$ is real, then so is the corresponding fundamental coweight $\varpi_\alpha$, which then gives a 1-parameter subgroup $\R^\times \to T_\tx{ad}(\R)$, which clearly acts simply transitively on $\mf{g}_\alpha(\R) \sm \{0\}$. If the simple root is complex, then a pair $(X_\alpha,X_{\sigma\alpha})$ contributing to the pinning is an element of $(\mf{g}_\alpha \oplus \mf{g}_{\sigma\alpha})(\R) = \C$. The fundamental coweight $\varpi_\alpha$ induces a 1-parameter subgroup $\C^\times \to T_\tx{ad}(\R)$ which again acts simply transitively on $(\mf{g}_\alpha \oplus \mf{g}_{\sigma\alpha})(\R)$.

Consider (2). Since all regular nilpotent elements of $\g(\R)$ are conjugate via $G(\R)$, the group $G_{\mathrm{ad}}(\R)$ acts transitively on
these orbits, and clearly the stabilizer of any such orbit is $\Ad(G(\R))$.

We already know the maps from (1) and (2) to (3) are $G_{\mathrm{ad}}(\R)$ equivariant. To finish the proof it is enough to show
the map $\P\mapsto \w_{\P,\Lambda}$ is a bijection. This is similar to the proof of (1). We leave the details to the reader; see
\cite[Lemmas 3.2 and 4.14]{ABV92}.
\end{proof}
...
$SL(2)$-triple $[X,H,Y]$ \cite[Chapter 3]{CM}.  The {\it Kosant Section} $\K(X)$ of
...
Suppose $\pi$ is an irreducible essentially discrete series representation and write $\pi=\pi(S,\tau)$ as in Section  \ref{sub:essds}. Let $H_\pi$ be the element of $i\s(\R)$ corresponding to $d\tau$ via $\kappa$. The $G(\R)$-conjugacy class $\O_\pi$ of $H_\pi$ is well defined. The purpose of this subsection is to prove the following result.

	

\begin{pro}
  \label{p:whittaker}
Let $\pi$ be a discrete series representation
and $\w=\w_X$ is a Whittaker datum for some regular nilpotent element $X\in i\g(\R)$.
Then $\pi$ is $\w$-generic if and only if $\O_\pi$ meets the Kostant section of $X$.
\end{pro}

We begin with some lemmas. If $W$ is a subset of a real or complex vector space $V$, define  $\AC(W)$,  the  {\it asymptotic cone} of $W$ as in   \cite[Proposition 3.7]{bvlocal}, \cite[Definition 2.9]{avav}:
$$
...
\begin{lem}
The group $Q(G(\R))$ acts without fixed points on the set of generic discrete series.
\end{lem}

\begin{proof}
Suppose $\pi=\pi(S,\tau)$ is a generic discrete series representation, and $g\in G_{\mathrm{ad}}(\R)$.
Since $S$ is unique up to conjugation by $G(\R)$, after modifying $g$ by an element of $\Ad(G(\R))$  we can assume $\Ad(g)(S)=S$,
and $\pi^g=(S,\Ad(g)\tau)$. Since $d\tau$ is regular, $\Ad(g)\tau$ is $G(\R)$-conjugate to $\tau$ if and only if $g\in G(\R)$,
i.e. $g=1$ in $Q(G(\R))$.

\end{proof}

\begin{lem}\label{l:unique}
Suppose $\pi$ is a generic discrete series representation. Also assume $\omega_\pi$ meets $\K(X)$ and $\K(Y)$
where $X,Y\in i\g(\R)$ are regular nilpotent elements. Then $Y$ is $G(\R)$-conjugate to $X$.
\end{lem}

\begin{proof}
Since $X,Y$ are regular nilpotent elements of $i\g(\R)$, $Y=\Ad(g)(X)$ for somem $g\in \Ad(G(\R))$. Therefore
$\O_\pi$ meets $\K(Y)=\K(\Ad(g)X)=\Ad(g)\K(X)$, which implies
$\O_{\pi^g}$ meets $\K(X)$. Since $\K(X)$ meets each regular complex orbit in a unique point, and $\O_\pi$ are $\O_{\pi^g}$ have the same complexification,
we conclude $\O_\pi=\O_{\pi^g}$. But this implies $\pi^g=\pi$. By the previous Lemma, viewing $g$ as an element of $Q(G(\R))$, this implies
$g\in \Ad(G(\R))$, i.e. $X,Y$ are $G(\R)$-conjugate.
\end{proof}
...
For the opposite implication suppose $\O_\pi$ meets $\K(Y)$ for some regular nilpotent element $Y$.
First we assume $\pi$ is $\w_X$-generic for some $X$.
By the previous argument we know $\O_\pi$
meets $\K(X)$; it is enough to show $Y$ is $G(\R)$ conjugate to $X$.
We proceed by contradiction. If $Y$ and $X$ are in different $G(\R)$-orbits, then by Lemma \ref{l:Q}
$\Ad(G(\R))(Y)=q(\Ad(G(\R)X)$ for some $q\ne 1$ in $Q(G(\R))$. Since $Q(G(\R))$ is acting by automorphisms on $G(\R)$,
$\pi^q$ is $\w_Y$-generic. By the previous argument this implies $\O_{\pi^q}$ meets $\kappa(Y)$.
But we have already assumed $\O_\pi$ meets $\kappa(Y)$. Recall $\kappa(Y)$ meets every regular complex orbit in exactly one point.
Since $\O_\pi$ and $\O_{\pi^q}=\Ad(q)\O_\pi$ are conjugate by $G(\C)$ this implies $\O_{\pi^q}=\O_\pi$.
Then $\pi^q\simeq\pi$, which contradicts Lemma \ref{l:Q}.

Now we drop the assumption that $\pi$ is generic.
In this case we will show that $\O_\pi$ does not meet $\K(X)$ for any regular nilpotent element $X$.
If $G(\R)$ is not
quasisplit there is nothing to prove, so assume $G(\R)$ is quasisplit.
Suppose $X$ is a regular nilpotent element.
There is a generic discrete
seires $\pi'$ such that $\O_\pi,\O_{\pi}$ have the same
complexification.  We know that $\O_{\pi'}$ meets $\K(Y)$ for some
regular nilpotent element $Y$. After acting by $Q(G(\R))$ if
necessary, we can assume $Y$ is $G(\R)$-conjugate to $X$, i.e. $\O_{\pi'}$ meets $\K(X)$. But we already know that
$\K(X)$ intersects each regular complex orbit in a single point, so $\O_{\pi}\cap \K(X)=\emptyset$.
...
It is now convenient to mention    a supplementary result, which is well known to the experts, but not easy to find in the literature.
...
We already proved the final assertion in Lemma \ref{l:unique}.

\begin{proof}
  Fix an elliptic torus $S$ and a regular nilpotent element $X\in i\g(\R)$. Consider the set of regular elements
  $i\s(\R)^*_{\mathrm{reg}}=\{\lambda \in i\s(\R)^*\mid \langle \lambda,\ch\alpha\rangle\ne 0$ for all roots $\alpha$.
  Consider the set
  $$
  \s_X=\{\lambda\in i\s(\R)^*_{\mathrm{reg}}\mid \K(X)\cap \Ad(G(\R))\lambda\ne\emptyset\}
  $$
  From properties of the Kostant section this is an open set \warn{check this!}.
  If the Weyl chamber of $\lambda$ does not have the property that all simple roots are non-compact then this set is empty.
  Otherwise suppose $X,Y$ are not $G(\R)$-conjugate. I claim $\s_X\cap \s_Y=\emptyset$.
\warn{Why? Well\dots this intersection can't contain an element of $\rho+X^*$, because then this discrete series representation
  would have two Whittaker models, contradicting Lemma \ref{l:unique}. Now argue by algebraicity and Zariski density\dots

This probably isn't worthwhile. I suggest we drop this Lemma, or else refer to AA}.
\end{proof}
...
\warn{this proof is not finished}
  ...
  \underline{Step 1:} Let $G(\R)'$ be the image of $G_\tx{sc}(\R) \to G(\R)$. Both sides are supported on $Z_G(\R) \cdot G(\R)'$, and transform by the same character of $Z_G(\R)$.
  ...
  \begin{proof}
	...
	Lemma \ref{lem:charsupp} shows that the character of a discrete series representation of $G(\R)$ is supported on $Z_G(\R) \cdot G(\R)'$. It follows that the function $\Theta_\varphi^{\mf{w},s}$ is supported on $Z_G(\R) \cdot G(\R)'$. Applying the same reasoning to $H$ we see that the function $S\Theta_{\varphi_\pm}$ is supported on $Z_{H(\R)_\pm} \cdot H(\R)_\pm'$, where $H(\R)_\pm'$ is the image of the natural splitting $H_\tx{sc}(\R) \to H(\R)_\pm$.
	
	

	To show that the right hand side of the identity of Theorem \ref{thm:main2} is supported on $Z_G(\R) \cdot G(\R)'$, consider a strongly regular semi-simple element $\delta \in G(\R)$ for which the right-hand side is non-zero. Thus there exists $\dot\gamma \in H(\R)_\pm$ such that $S\Theta_{\varphi'}(\dot\gamma) \neq 0$ and $\Delta(\dot\gamma,\delta)\neq 0$.
	
	Let $T \subset G$ be the centralizer of $\delta$ and let $S \subset H$ be the centralizer of the image $\gamma \in H(\R)$ of $\dot\gamma$. Since $S\Theta_{\varphi'}(\dot\gamma) \neq 0$ we know $\gamma \in Z_H(\R) \cdot S(\R)'$, where $S(\R)'$ is the image of $S_\tx{sc}(\R) \to S(\R)$, and $S_\tx{sc}$ is the preimage of $S$ in $H_\tx{sc}$. Since $\Delta(\dot\gamma,\delta)\neq 0$ there is a (necessarily unique) admissible isomorphism $j : S \to T$ mapping $\gamma$ to $\delta$. The induced isomorphism $X_*(S) \to X_*(T)$ identifies the coroot lattice $Q^\vee(S,H)$ with a sublattice of the coroot lattice $Q^\vee(T,G)$, and hence restricts to a homomorphism $X_*(S_\tx{sc}) \to X_*(T_\tx{sc})$. This means that $j : S(\R) \to T(\R)$ maps $S(\R)'$ into $T(\R)'$. This reduces to showing that $j$ maps $Z_H(\R)$ to $Z_G(\R) \cdot T(\R)'$, for which it is enough to show that the map $(S/Z_G)(\R) \to (T/Z_G)(\R)$ sends $(Z_H/Z_G)(\R)$ into the image of $T_\tx{sc}(\R) \to T_\tx{ad}(\R)$; here $T_\tx{sc}$ is the preimage of $T$ in $G_\tx{sc}$ and $T_\tx{ad}$ is the image of $T$ in $G_\tx{ad}$.
...
This allows us to assume that $G$ is semi-simple and simply connected in what follows, and hence apply the results of \cite{HCDSI}.
  ...
  \underline{Step 2}: Both sides are invariant eigendistributions, with the same infinitesimal character, whose values on the regular set are bounded.
	...
	\begin{proof}
		...
		It is known \cite[Theorem 3]{HCDSI} that the un-normalized character of a discrete series representation $\pi$ of a $G(\R)$ is represented by a conjugation-invariant function $\Theta$ which is an eigenfunction for the center $\mf{z}$ of the universal enveloping algebra with respect to the character $\mu : \mf{z} \to \C$ corresponding to $d\tau$, where $(S,\tau)$ is the Harish-Chandra parameter of $\pi$, and such that the normalized function $|D_T|^{1/2}\Theta$ is bounded. These statements carry over to the left-hand side of the identity of Theorem \ref{thm:main2}, which is just a complex linear combination of such functions.
		...
		To see boundedness, we note first that we have already normalized the characters by $|D_T|^{1/2}$, so we need to show that the right-hand side is bounded. For any fixed $\delta$, there are only finitely many $\dot\gamma$ with $\Delta(\dot\gamma,\delta) \neq 0$, and in fact their number is bounded by $|\Omega(T,G)|$. The values of the transfer factor are roots of unity (\cite[Lemma 4.3.3]{KalHDC}). The boundedness of the right-hand side now follows from the boundedness of $S\Theta_{\varphi'}$.

		To check the eigendistribution property for the function $\delta \mapsto \sum_\gamma \Delta(\dot\gamma,\delta)S\Theta_{\varphi'}(\dot\gamma)$ we fix $\delta$ and let $T \subset G$ be its centralizer. To apply a differential operator to this function at $\delta$, it is enough to consider the function in a neighborhood of $\delta$. Let $\gamma_1\dots,\gamma_n$ be representatives for the stable classes in $H(\R)$ that transfer to the stable class of $\delta$ and let $S_1,\dots,S_n \subset H$ be their centralizers. Let $j_i : T \to S_i$ be the unique admissible isomorphism with $j_i(\delta)=\gamma_i$. There is a connected open neighborhood $U \subset T(\R)$ of $\delta$ so that, for each $\delta' \in U$, $j_1(\delta'),\dots,j_n(\delta')$ is a set of representatives for the stable classes in $H(\R)$ that transfer to $\delta'$, and such that the double cover $S_i(\R)_x$ splits over $j_i(U)$. We let $U_i \subset S_i(\R)_x$ be an arbitrary choice of one of the two connected components of the preimage of $j_i(U)$ in $S_i(\R)_x$. Then $j_i$ lifts to an isomorphism $U \to U_i$, which we still call $j_i$. The restriction to $U$ of the function $\delta \mapsto \sum_\gamma \Delta(\dot\gamma,\delta)S\Theta_{\varphi'}(\dot\gamma)$ is now given by $\delta' \mapsto \sum_{i=1}^n \Delta(j_i(\delta'),\delta')S\Theta_{\varphi'}(j_i(\delta'))$.

		It will be enough to consider each summand individually. By \cite[Corollary 4.3.4]{KalHDC}, the function $\delta' \mapsto \Delta(j_i(\delta'),\delta')$ is locally constant, so after possibly shrinking $U$ we may assume that it is constant. Thus the summand under consideration becomes $\delta' \mapsto S\Theta_{\varphi'}(j_i(\delta'))$. The infinitesimal character of this function is the transport under $j_i : \mf{t} \to \mf{s}_i$ of the infinitesimal character of $S\Theta_{\varphi'}$. We claim that this transport equals the infinitesimal character of $\Theta_{\varphi}^{\mf{w},s}$.

\warn{Jeff, here I need the following statement: Let $F : G(\R)_\tx{sr} \to \C$ be a smooth class function. Let $X \in \mc{Z}(\mf{g})$. For a maximal torus $T \subset G$ let $h : \mc{Z}(\mf{g}) \to \mc{U}(\mf{t})^{\Omega(T,G)}$ be the Harish-Chandra isomorphism. Then, for any $\delta \in T(\R)$ we have $(XF)(\delta) = (h(X)F)(\delta)$.}
\end{proof}
