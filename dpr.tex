...
and $\eta$ is a non-degenerate character of $N(\R)$. We  write $\w=[(B,\eta)]$ for it $G(\R)$-conjugacy class of $(B,\eta)$.
...
Let $\kappa$ be the Killing form. Given $B,N,T$ as above let $\overline N$ be the opposite nilradical with respect to $T$. 
...
Let $H_\pi$ be the element of $i\s(\R)$ corresponding to $d\tau$ via the Killing form.
...
Kostant showed \cite{kostant} that the Kostant section meets every regular orbit in a unique point.

The Kostant section $\K(X)$ depends on a choice of
triple, but any two such choices are $G(\R)$-conjugate, and the
$G(\R)$-conjugacy class of $\K(X)$ only depends on the $G(\R)$-conjugacy class of
$X$.

Suppose $X\in \g(\R)$ and $\O$ is a regular nilpotent orbit which is defined over $\R$.
Then, since $X$ is also defined over $\R$, the unique point in $\K(X)\cap \O$ is contained in $\g(\R)$.
...
By \cite{Kos63} $\K(Y)$ is transverse to $\O_X$ for any $Y\in \O_X$.
...
By \cite[Theorem A]{matumoto} $X\in \WF(\pi)$. By \cite{harris} $\WF(\pi)=\AC(G(\R)\cdot H_\pi)$.
...
Set $G=\SL(2,\R)$. Let $\s(\R)=\{t_x\mid x\in\R\}$ where
$$
t_x=\begin{pmatrix}0&x\\-x&0
\end{pmatrix}
$$

Then $\alpha(t_x)=2ix$ is the positive root, and
$$
X_\alpha=\frac12\begin{pmatrix}1&i\\i&-1
\end{pmatrix}
$$
Then $X_{-\alpha}=\overline{X_\alpha}$, and $[X_\alpha,X_{-\alpha}]=t_{-i}$.
The Killing form satisfies
$$
\kappa(t_x,t_y)=-8xy.
$$
For $z\in \C$ define $\lambda_z\in \s^*$ by $\lambda_z(t_x)=xz$.
Then $\lambda_{ik}$ is the Harish-Chandra parameter of a discrete series $\pi(\lambda_{ik})$ for $k\ne 0$.

Define $H_{\lambda}\in \s^*$ so that $\kappa(H_\lambda,t_x)=\lambda(t_x)$. Then $H_{\lambda_z}=t_{-\frac z8}$:
$$
...
\end{pmatrix}=\{\begin{pmatrix}0&i\\iy&0
\end{pmatrix}\mid y\in\R\}
$$
In particular
$
\K\begin{pmatrix}0&-i\\0&0\end{pmatrix}
$
meets
$
G(\R)H_{\lambda_{ik}}=
\begin{pmatrix}0&-\frac{ik}8\\\frac{ik}8&0\end{pmatrix}
$
for $k>0$ as required by Proposition \ref{p:whittaker}
Similarly
$
\K\begin{pmatrix}0&i\\0&0\end{pmatrix}
$
