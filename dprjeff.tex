...
\end{lem}
\begin{proof}
Let $\lambda=d\tau\in i\s(\R)^*$, and write $\mf{w}=\mf{w}_X$ for
$X\in i\g(\R)^*$.  By Proposition \ref{p:whittaker} $\pi_j$ isi
$\mf{w}$-generic if and only if $j(\gamma)$ meets the Kostant section
$\K(X)$.  Therefore $j(\gamma)$ meets any Kostant section for
precisely one $j \in J^{G_0}(\R)/G_0(\R)$. At the same time, $j
\mapsto \pi_j$ is a bijection from $j \in J^{G_0}(\R)/G_0(\R)$ to
$\Pi_\varphi((G_0,1,1))$ by construction of the latter.
\end{proof}
...
\bibliography{bibliography.jeff}
