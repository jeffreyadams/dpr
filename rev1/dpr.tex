...
\end{equation}
of the character identities, where $\pi \mapsto \<\pi,-\>$ is an injection of $\Pi_\varphi(G)$ into the character group $(S_\varphi/Z(\hat G)^\Gamma)^*$ of the finite (as it turns out abelian) group $S_\varphi/Z(\hat G)^\Gamma$, uniquely determined to make the above identity true, and $\Delta(\gamma,\delta)$ is the Whittaker normalization of the geometric transfer factor.
...
The notion of a rigid inner twist is again sufficiently rigid, and in addition the equivalence classes it induces coincide with those induced by the notion of inner twist. This will be the notion that we will use.
...
An element $\gamma \in T(\R)$ is called regular if $T$ is the identity component of its centralizer, and strongly regular if $T$ is its centralizer. In either case, $T$ is uniquely determined by $\gamma$, and we can write $D_T(\gamma)=D(\gamma)$.
...
Under the isomorphism $\mb{S}^1 \to \C^\times/\R_{>0}$ the map $z/\bar z$ becomes the squaring map, and we see that $T(\R)_{\pm\pm}$ is equipped with a character $\alpha^{1/2} : T(\R)_{\pm\pm} \to \mb{S}^1$ for each root $\alpha$, and that $\beta^{1/2}=(\alpha^{1/2})^{-1}$ whenever $\beta=\alpha^{-1}$. There is an obvious surjective homomorphism $T(\R)_{\pm\pm} \to T(\R)_\pm$ whose kernel is the kernel of the multiplication map $\prod\{\pm1\} \to \{\pm1\}$. The function $\alpha^{1/2}-\alpha^{-1/2}$ is well-defined on the big cover, while for any choice Borel $\C$-subgroup $B$ the function
...
We will recall here the definition of essentially discrete series representations of real reductive groups and their classification due to Harish-Chandra. We will need to allow for a slightly different set-up than that in Harish-Chandra's original papers.
...
We may thus call the subgroup $\mc{S}$ in the statement of the above lemma an ``elliptic maximal torus'' of $\mc{G}$.
...
-Chandra \cite[Theorem 27.5]{Kot05}
...
defined by $\tx{Lift}(d)(f)=d(f^{H_\pm})$ resp. $\tx{Lift}(d)(f)=d(f^{H_1})$, where $f^{H_\pm}$ resp. $f^{H_1}$ are functions matching $f$ as in Definition \ref{dfn:matching}.
\end{dfn}
...
\[ 
\begin{aligned}
[|\nu_a|^{-1/2} \circ \gamma(z) \circ |\nu_a|^{1/2}]\circ L_{a^{-1}}(f)(1) &= L_{a^{-1}}\circ(|\Delta_T|^{-1/2} \circ \gamma(z) \circ |\Delta_T|^{1/2})(f)(1)\\
...
\end{lem}
\begin{proof}
	This follows from the fact that any two elliptic maximal tori of $G$ are conjugate under $G(\R)$ -- this is a well-known fact that is seen as follows. It is enough to show conjugacy under $G_\tx{der}(\R)$, so we may assume $G$ is semi-simple, hence $S$ is anisotropic. Then $S(\R)$ is compact \cite[\S24.6(c)]{Bor91}, hence contained in a maximal compact subgroup of $G(\R)$. But any two maximal compact subgroups of $G(\R)$ are conjugate \cite[\S24.6(a)]{Bor91}, and any two maximal tori of a compact Lie group are conjugate \cite[Corollary 4.35]{KnappLie}.
\end{proof}
...
\subsection{Generic essentially discrete series representations} \label{sub:gen}
...
\begin{cor}
	Let $\pi$ be an eds representation. There exists at most one Whittaker model $\mf{w}$ for which $\pi$ is $\mf{w}$-generic.
\end{cor}
We remark that the analogous statement for $p$-adic groups is false, as can already be seen in the example of $G=\tx{SL}_2$, which for $p\neq 2$ has four Whittaker data and each regular supercuspidal representation is generic for exactly two of them.
...
(1) Let $\O_\lambda = G(\R) \cdot \lambda$ and recall $\O_\pi=G(\R)\cdot d\tau$.  According to Proposition
\ref{p:whittaker}, we need to show that $\O_\lambda$ meets $\K^*(X)$
if and only if $\O_{d\tau}$ meets $\K^*(X)$. Applying Lemma \ref{lem:g1}
we see that we need to show the equivalence of $X \in \AC(\O_\lambda)$
and $X \in \AC(\O_{d\tau})$. This follows since $\lambda,d\tau$ are in the same Weyl chamber and the fact
\cite[Proposition 3.9]{adams_afgoustidis_whittaker}
that $\AC(\O_\lambda)$ only depends on the Weyl chamber of $\lambda$.
%What we written before was correct; I've modified the wording slightly to make the argument clearer
...
The following is the main theorem of this article. It is a fundamental result of Shelstad \cite{She82}, \cite{SheTE3}, \cite{SheTE2}.
...
\begin{proof}
  ...
  \underline{Step 1:} Let $G(\R)^\natural$ be the image of $G_\tx{sc}(\R) \to G(\R)$. Both sides of the identity of Theorem \ref{thm:main2} are supported on $Z_G(\R) \cdot G(\R)^\natural$, and transform by the same character of $Z_G(\R)$. 
...
This allows us to assume that $G$ is semi-simple and simply connected in what follows, and hence apply the results of \cite{HCDSI}.
  ...
  \underline{Step 2}: Both sides of the identity of Theorem \ref{thm:main2} are invariant eigendistributions, with the same infinitesimal character, whose values on the regular set are bounded.  
...
\end{lem}
\begin{proof}
This is the property \eqref{eq:tfd3} that was reviewed in an idealized situation. We say here a few more words about the actual situation in the setting of double covers. We have the commutative diagram
\[ \xymatrix{
...
\]
Here $^LS_\pm$ is the $L$\-group of the double cover $S(\R)_\pm$ and the bottom horizontal map is the canonical $L$\-embedding. We have written $^LS_{H,\pm}$ for the $L$\-group of the double cover of $S_H(\R)$ obtained as the Baer sum of the canonical double cover $S_H(\R)_H$ coming from $R(S_H,H) \subset X^*(S_H)$ and the double cover $S_H(\R)_\pm$ that is the preimage of $S_H(\R)$ in the double cover $H(\R)_\pm \to H(\R)$. The top horizontal map is the canonical $L$\-embedding of the $L$\-group of $S_H(\R)_H$ into $^LH$, which remains an $L$\-embedding after passing to the $\pm$-covers. The right vertical map is the canonical $L$\-embedding $^LH_\pm \to {^LG}$. The map $a$ is the unique map making the square, hence also the triangle, commute. It is the parameter of a genuine character of the double cover of $S(\R)$ obtained as the Baer sum of $S_H(\R)_H$, $S_H(\R)_\pm$, and $S(\R)_\pm$, where we have identified $S_H$ with $S$. This Baer sum is the same as for $S_H(\R)_\pm$ and $S(\R)_{G/H}$. 
\end{proof}
